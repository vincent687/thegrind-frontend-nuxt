<template>
  ...
  <div class="col-start-2 flex">
    <div class="grid grid-cols-1 w-48w ml-[3vw]">
      ...
      <div v-if="companyState.status != 'success'">No record</div>

      <div v-else>
        <div
          class="flex"
          v-for="rowIdx in Math.ceil(companyState.data.length / getScreenRow)"
          :key="rowIdx"
        >
          <div
            class="one-third column"
            v-for="company in companyState.data.slice(
              getScreenRow * (rowIdx - 1),
              getScreenRow * rowIdx
            )"
            :key="company.id"
          >
            <NuxtLink :to="`/explore/${company.id}`">
              <AvatarCard
                class="m-9"
                :company="company"
...
const getScreenRow = computed(() => {
  if (process.client) {
    ...
    if (window.innerWidth >= 1149 && window.innerWidth < 1556) {
      return 3
    } else if (window.innerWidth >= 1556 && window.innerWidth < 1915) return 4
    else {
      return 5
    }
  } else {
