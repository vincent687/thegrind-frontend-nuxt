      ...
      ></iframe>
        ...
        <div class="grid grid-cols-1">
          <NuxtLink :to="`/my-teams/exercise-${route.params.groupId[0]}/5`"  >   <ExerciseCard   :video="lessonState.data?.videos[count]" /> </NuxtLink>
            <div>
                <IconPrevious @click="getPrevVideo" v-if="count != 0" class="inline-block" />
                <IconNext   @click="getNextVideo" v-if="lessonState.data?.videos.length -1 > count" class="inline-block float-right" />
            </div>
        </div>
