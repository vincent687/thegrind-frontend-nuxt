...
const getYoutubeUrl = computed(() => {
  if (videoState.value.status === 'success') {
    let result = videoState.value.data?.url.replace(
      'https://youtu.be/',
  ...
  <div class="col-start-2 grid">
    ...
    >
      ...
      <iframe
        ...
        class="rounded-t-[25px] w-[87vw] lg:w-[40vw] xl:w-[40vw] 2xl:w-48w"
        allowfullscreen="1"
        width="640"
        height="360"
        :src="getYoutubeUrl"
      ...
      <CommentCard :video="videoState">
        ...
        <div
          v-if="getMobile && getCurrentTab == 2"
          class="
            grid grid-cols-1
            xl:w-[24vw]
            2xl:w-[22vw]
        ...
        >
          <NuxtLink
            :to="`/my-teams/exercise-${route.params.groupId[0]}/5`"
            v-if="otherVideosState.data"
          >
            <ExerciseCard :video="otherVideosState?.data[count]" />
          </NuxtLink>
          <div v-if="otherVideosState.data" class="m-auto">
            ...
            <IconNext
              @click="getNextVideo"
              v-if="otherVideosState.data.length - 1 > count"
              class="inline-block"
            />
          </div>
        </div>
