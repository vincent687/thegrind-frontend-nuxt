...
<template>
 <!-- {{course}} -->
      <div class="relative container grid grid-cols-1 rounded-[25px] bg-white w-[17rem] h-[20rem] 2xl:w-112 2xl:h-112 shadow-md " @click="$emit('click', course)">
          <div class="absolute h-3/6 top-0 w-[17rem]  h-[20rem] 2xl:w-112">
 
              <img  class="w-full rounded-t-[25px]  object-cover mb-3"  alt="abc" :src="course?.course?.attachment.url" />
                <!-- <img
          ...
          <div class="absolute h-3/6 2xl:top-2/3 left-4 top-[54.66%]">
                <p class="font-bold my-2 text-base 2xl:text-lg">{{course?.name}}</p>
